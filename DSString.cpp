...
bool DSString::operator==(const char* arg) const {
    if(s == nullptr || arg == nullptr){
        // if one is nullptr, only equal if both are nullptr
        return(s == nullptr && arg == nullptr);
    }
...
char &DSString::operator[](const int n) {
    ...
    throw std::out_of_range("Error, trying to access memory out of range of string");
}
...
DSString DSString::substring(int start, int numChars) {
    ...
    if((start + numChars) > strlen(s)){
        throw std::out_of_range("Error, substring is out of range of string");
    }
...
void DSString::AddChar(char newChar, int location) {
    if(location > this->getLength() + 1){
        throw std::out_of_range("not a valid index for the string");
    }
    char* new_s = new char[this->getLength() + 1];
    strncpy(new_s, s, location);
    new_s[location] = newChar;
    strcat(new_s, s + location);
    delete[] s;
    s = new_s;
